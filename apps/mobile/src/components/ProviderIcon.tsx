...
/** Meta symbol, as dev.meta.ai brands Muse Code. The art is 150x100; the box is padded square. */
function MuseIcon({ size }: { readonly size: number }) {
  const id = useId().replace(/:/g, "");
  return (
    <Svg width={size} height={size} viewBox="0 -25 150 150" fill="none">
      <Defs>
        <LinearGradient
          id={`${id}-a`}
