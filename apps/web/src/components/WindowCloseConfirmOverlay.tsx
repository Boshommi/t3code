...
/**
 * Chrome/VS Code-style "press again to close". Native File → Close no longer
 * owns Cmd+W; when no terminal or right-panel tab is open this overlay arms
...
export function WindowCloseConfirmOverlay() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.defaultPrevented && !(event.key === "Escape" && visibleRef.current)) {
        return;
      }
      const command = resolveShortcutCommand(event, keybindings, {
        context: {
          ...
          rightPanelOpen: isWindowCloseRightPanelOpen(),
        },
      });
